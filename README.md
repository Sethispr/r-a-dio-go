...
Barebones implementation for infinite [r-a-d.io](https://r-a-d.io/search) song requests with no 30 minute limits (please use responsibly), this cli simply gets proxies > read sites source code to scrape the amazing one time [gorilla/csrf](https://github.com/gorilla/csrf) token and as a non malicious user tricks the server to accept your song requests. qol features may be worked on soon.
